		...
		<table width="100%" border="0" cellspacing="0">
			<tr>
				...
				<td><div style="text-align:right; margin:5px 10px 5px 0px">
					...
					<form id="orderForm" action="${pageContext.request.contextPath }/creatOrderServlet" method="post">
							...
							<tr>
								<td><table width="100%" border="0" cellspacing="0">
										...
										<tr>
											...
											<c:set var="count" value="0"></c:set><!-- 循环一次计算一下小计，初始值为0 -->
									<c:forEach items="${cart }" var="p" varStatus="vs">
												<table width="100%" border="0" cellspacing="0">
													<tr>
														<td width="10%">${vs.count }</td>  
<!-- 从session的购物车中取值，，session中有map，map中有key -->	<td width="40%">${p.key.name }</td>
	...
	<!-- 第一次为0，加上小计之后就不为0，所以要这样写 -->		      <c:set var="count" value="${count+p.key.price*p.value }"></c:set>
													</tr>
												</table>
									</c:forEach>
								...
								<!-- 这里输出结果 -->						<font style="color:#FF0000">合计：&nbsp;&nbsp;${count }元</font></td>
													</tr>
		<!-- 传入count值 -->					<input type="hidden" name="money" value="${count }"/>
												</table>
